...
- Real CERN signal/background dataset using records `7901` and `7779`
- Real CERN feature-engineering validation mode
- Real CERN baseline training and evaluation through `dataset_mode="real_cern"`
- Leakage review and leakage-reduced real CERN feature set
...
```text
Raw / sample collider data OR real CERN/Open Data ROOT files
        ↓
Local PySpark ETL for sample data + ROOT adapter ETL for real CERN data
        ↓
Processed Parquet / CSV datasets
        ↓
...
├── etl/
│   ├── adapters/
│   │   └── root_adapter.py       # ROOT inspection, branch probing, feature extraction
│   ├── cern_client.py            # CERN Open Data metadata and file-location registration
│   ├── search_cern_records.py    # CERN Open Data search and candidate enrichment
│   ├── real_cern_etl_job.py      # Real CERN ROOT → processed/curated CSV pipeline
...
│   ├── logistic_regression_sample_collider.joblib
│   ├── random_forest_sample_collider.joblib
│   ├── logistic_regression_real_cern.joblib
│   ├── random_forest_real_cern.joblib
│   └── model_metadata.json
│
├── training/
...
Current extracted real CERN processed columns include:
...
Current real CERN v2 status: the default real CERN ETL job combines CMS Higgs-to-gamma-gamma signal record `7901` with CMS GJets background record `7779`, producing a two-class curated dataset that is trainable through `dataset_mode="real_cern"`.

---

## Real CERN Baseline Training and Leakage Review

ColliderOpsAI now supports baseline model training and evaluation on a real CERN/Open Data signal-vs-background dataset.

Default real CERN dataset configuration:

```text
Signal:     CMS Higgs-to-gamma-gamma Monte Carlo record 7901
Background: CMS GJets Monte Carlo record 7779
Rows:       200 total rows, balanced as 100 signal and 100 background
Mode:       dataset_mode="real_cern"
```

Initial training with all extracted features produced perfect evaluation metrics. A leakage review identified `gen_event_signal_process_id` as a process-identifying feature that allowed the model to separate signal and background too easily.

To reduce leakage, `gen_event_signal_process_id` is retained in processed data for traceability but excluded from the curated training feature set.

Leakage-reduced real CERN baseline metrics:
...
This is a more realistic baseline than the earlier perfect metrics and confirms that the pipeline is learning from a smaller, leakage-reduced feature set rather than a process identifier shortcut.

Current real CERN training commands:
...
Training now consumes the curated dataset generated by the local ETL pipeline or the real CERN/Open Data ETL pipeline.
...
The training feature-engineering layer supports two dataset modes:
...
`real_cern` mode uses the curated signal/background CSV generated from real CERN ROOT files and applies a leakage-reduced feature set.
...
Evaluation now consumes the curated dataset generated by the local ETL pipeline or the real CERN/Open Data ETL pipeline.

Evaluation outputs are saved under:

```text
evaluation_metrics/model_comparison_<dataset_mode>_<timestamp>.csv
```
...
- Real CERN dataset schema validation can be run manually through `validate_model_ready_dataset(..., dataset_mode="real_cern")`
- Real CERN binary training readiness is true after combining signal record `7901` and background record `7779`
...
- Real CERN/Open Data ROOT ingestion
- ROOT branch probing and readable event feature extraction
- Leakage review and leakage-reduced baseline modeling
...
- CERN Open Data metadata registration through `cernopendata-client`
- CERN record search and file-location enrichment
- Dataset registry for source metadata and ROOT file URLs
- ROOT adapter for file inspection, Events tree discovery, branch probing, and readable feature extraction
- Real CERN processed CSV output
- Real CERN curated ML-ready CSV output
- Real CERN signal/background dataset using records `7901` and `7779`
- Real CERN feature-engineering validation
...
- Dataset versioning and lineage
- Richer non-leaky ROOT feature extraction
- Expansion across more CERN ROOT files and/or background records
...
- The project now includes real CERN/Open Data ROOT ingestion and a two-class signal/background dataset, but the current feature set is still small and should be treated as a prototype baseline rather than a scientifically complete classifier.
- Model performance metrics are still early-stage because the real CERN feature set is limited and based on a small prototype sample.
...
- Some complex CMS AODSIM branches cannot be directly deserialized by `uproot`, so v1 extracts a controlled subset of readable branches only.
- Initial perfect metrics were caused by leakage-prone process-identifying features; `gen_event_signal_process_id` is now excluded from the training feature set.

---

## Near-Term Next Steps

1. Extend ROOT feature extraction with richer non-leaky event-level features
2. Expand the real CERN dataset across more file indexes and/or additional background records
3. Analyze leakage-reduced model performance and feature importance
4. Compare Logistic Regression, Random Forest, and additional baseline models on `dataset_mode="real_cern"`
5. Add tests for CERN client, ROOT adapter helpers, and real CERN ETL row generation
6. Add model explainability with feature importance / SHAP
7. Add an agent API endpoint for LangGraph workflows
8. Add external CERN/Open Data documentation to RAG
9. Add AWS Glue/S3 adaptation for the local and real CERN ETL pipelines
10. Clean up Pydantic and LangChain deprecation warnings